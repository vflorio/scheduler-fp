---
sidebar_position: 3
---

# Device Registry

The Device Registry is the central catalogue of all lab devices. It associates a **label** and an **IP address** to each device, grouped into three categories with distinct purposes.

## Device Categories

```mermaid
---
config:
  flowchart:
    nodeSpacing: 40
    rankSpacing: 60
---
graph TD
    Registry["Device Registry"]

    Registry --> CU["Control Units"]
    Registry --> AC["Android Cameras"]
    Registry --> TV["Smart TVs"]

    CU --> CUD1["device"]
    CU --> CUD2["device"]
    AC --> ACD1["device"]
    TV --> TVD1["device"]

    style Registry fill:#9b59b6,color:#fff
    style CU fill:#f5a623,color:#fff
    style AC fill:#4a90d9,color:#fff
    style TV fill:#2ecc71,color:#fff
    style CUD1 fill:#f5a623,color:#fff
    style CUD2 fill:#f5a623,color:#fff
    style ACD1 fill:#4a90d9,color:#fff
    style TVD1 fill:#2ecc71,color:#fff
```

### Control Units

CandyBoxes and Raspberry Pis that manage groups of downstream devices. Each control unit **owns** one or more devices (TVs, set-top boxes, etc.).

Used for:
- **Bulk-offline heuristic** - when multiple devices owned by the same control unit go offline simultaneously, recovery targets the control unit itself rather than individual devices
- **Dashboard** - show control unit health and owned-device status

### Android Cameras

Android devices running the Suitest camera app. The lab network is large - the registry defines the **exact subset of IPs** the bridge should operate on, ignoring all other ADB-reachable devices.

Used for:
- **Automated recovery** - restart camera app, reboot device, verify stream availability
- **Scoped discovery** - only mDNS endpoints matching a registered IP are processed by the AndroidBridge

### Smart TVs

Lab TVs used for testing. Not actively managed by the current automation, but tracked for future use.

Used for:
- **Scheduled TV Recording Sessions** (planned) - power on, select input, record, power off

## Device Model

The registry is split into three buckets (`devices.controlUnits`, `devices.cameras`, `devices.tvs`) instead of a
single flat list with a `category` field. Control units and downstream devices come from **different Suitest
entities** with different identities - a control unit has no IP address in the Suitest API, it's identified by
`id` - so forcing them into one shared shape was the root cause of control units silently failing to sync.

| Bucket | Identity | Fields |
|---|---|---|
| `controlUnits` | `id` | `id`, `label`, `online`, `controlled` |
| `cameras` | `ip` | `label`, `ip`, `controlled` |
| `tvs` | `ip` | `label`, `ip`, `controlled` |

Cameras and TVs are kept as separate types (not a shared `Device` model) since their fields are expected to
diverge as camera- and TV-specific features are built out.

## Role in the System

The service operates **in parallel** on control-unit devices and android-camera devices with separate workflows and automation policies. The registry is the single source of truth that feeds both pipelines:

```mermaid
---
config:
  flowchart:
    nodeSpacing: 30
    rankSpacing: 60
---
graph TD
    Registry["Device Registry"]

    Registry -- "control-unit IDs" --> CUPipeline["Control Unit Pipeline <br/> (Suitest API workflows)"]
    Registry -- "android-camera IPs" --> ACPipeline["Android Camera Pipeline <br/> (ADB workflows)"]
    Registry -- "smart-tv IPs" --> TVPipeline["TV Pipeline <br/> (planned)"]

    style Registry fill:#9b59b6,color:#fff
    style CUPipeline fill:#f5a623,color:#fff
    style ACPipeline fill:#4a90d9,color:#fff
    style TVPipeline fill:#2ecc71,color:#fff
```

## Implementation: Suitest

The registry is currently populated from the **Suitest Public API** (v4). Device and control-unit data is fetched, paginated automatically, and mapped to the minimal internal model.
